...
class Environment(ShowBase):
    ...
    def calcPos(self, src, dst, connection):
        ...
        heading = ORIENTATION[connection.dst.orientation]
        # heading/orientation of dst model
        if 'Hinge' not in connection.dst.type and 'Hinge' not in connection.src.type:
            #if its brick to brick
            print("BRICK TO BRICK")
            if connection.src.orientation  == 0:
                if connection.src_slot == 3:
                    dst.setHpr(0, -90, heading)
                else:
                    dst.setHpr(0, 0, -heading)
            elif connection.src.orientation ==1:
                if connection.src.direction ==2:
                    dst.setHpr(-heading, 0, heading)
                elif connection.src.direction==1:
                    dst.setHpr(-180, 0, -heading)
            elif connection.src.orientation ==2:
                dst.setHpr(90, heading, 0)
            elif connection.src.orientation ==3:
                heading = ORIENTATION[connection.dst.orientation+3]
                dst.setHpr(90, 0, heading)

        if 'Hinge' in connection.src.type:
            #if the source is a hinge
            connection.dst.direction = connection.src.direction
            if connection.src.direction == 1:
                dst.setHpr(0, 180, heading)
            elif connection.src.direction == -1:
                dst.setHpr(0, 0, heading)
            elif connection.src.direction == 2:
                if (connection.dst.orientation == 0):
                    dst.setHpr(90, 0, heading + 180)
                elif(connection.dst.orientation == 1):
                    dst.setHpr(0, 90, heading)
                    print("Changing")
                else:
                    dst.setHpr(90, 0, heading - 180)
            elif connection.src.direction == -2:
                dst.setHpr(0, 270, heading)
            elif connection.src.direction == 3:
                if (connection.dst.orientation == 1 or connection.dst.orientation == 0) and 'Hinge' in connection.dst.type:
                    dst.setHpr(90, 0, heading + 180)
                elif 'Hinge' in connection.dst.type:
                    dst.setHpr(90, 0, heading - 180)
                else:
                    dst.setHpr(90, 0, heading)
            elif connection.src.direction == -3:
                if 'Hinge' in connection.dst.type:
                    dst.setHpr(270, 0, heading)
                else:
                    dst.setHpr(270, 0, heading)
        elif 'Hinge' not in connection.src.type:
            #if the source is a brick
            if connection.src_slot == 0:
                #if source slot is 0, don't take orientation into consideration
                connection.dst.direction = 1;
                if 'Hinge' in connection.dst.type and connection.src.orientation == 0:
                    dst.setHpr(-180,0,-heading)
                elif 'Hinge' in connection.dst.type and connection.src.orientation == 1:
                    dst.setHpr(0,180,heading)
                elif 'Hinge' in connection.dst.type and connection.src.orientation == 2:
                    dst.setHpr(180, 0, heading)
                elif 'Hinge' in connection.dst.type and connection.src.orientation == 3:
                    dst.setHpr(0, 180, heading)
            if connection.src_slot == 1:
                if 'Hinge' in connection.dst.type and connection.src.orientation == 0:
                    dst.setHpr(0, 90, heading)
                elif 'Hinge' in connection.dst.type and connection.src.orientation == 1:
                    if (connection.dst.orientation == 1 or connection.dst.orientation == 0):
                        dst.setHpr(90, 0, heading + 180)
                    else:
                        dst.setHpr(90, 0, heading - 180)
                elif 'Hinge' in connection.dst.type and connection.src.orientation == 2:
            ...
            if connection.src.orientation == 0:
                if connection.src_slot == 1:
                    connection.dst.direction = 2+connection.src.orientation
                elif connection.src_slot == 2:
                    if connection.src.direction ==1:
                        connection.dst.direction = -connection.src.direction
                    else:
                        connection.dst.direction = connection.src.direction
                elif connection.src_slot == 3:
                    connection.dst.direction = -2-connection.src.orientation
            elif connection.src.orientation == 1:
                if connection.src_slot == 1:
                    connection.dst.direction = -3
                elif connection.src_slot == 3:
                    connection.dst.direction = -2-connection.src.orientation
                elif connection.src_slot == 2:
                    connection.dst.direction = connection.src.direction
            elif connection.src.orientation == 2 or connection.src.orientation == 3:
                if connection.src_slot == 1:
                    if connection.src.direction == -2:
                        connection.dst.direction = 3
                    else:
                        connection.dst.direction = -(connection.src.orientation)
                elif connection.src_slot == 3:
                    connection.dst.direction = connection.src.orientation
                elif connection.src_slot == 2:
                    connection.dst.direction = connection.src.direction
        ...
        print("DIRECTION",connection.dst.direction)

        if connection.dst.direction == 1:                                               # use src slot to determine which side to place dest model
            dst_pos = src_pos + LVector3f(0, -(src_dim + dst_dim), 0)
        elif connection.dst.direction == 2:
            dst_pos = src_pos + LVector3f(0, 0, src_dim + dst_dim)
        elif connection.dst.direction == -1:
            dst_pos = src_pos + LVector3f(0, src_dim + dst_dim, 0)
        elif connection.dst.direction == -2:
            dst_pos = src_pos + LVector3f(0, 0, -(src_dim + dst_dim))
        elif connection.dst.direction == 3:
    ...
    def traverseTree(self, robot):
        ...
        for connection in robot.connections:
            ...
            if connection.src.root:
                ...
                #set orientation of source
                heading = ORIENTATION[connection.src.orientation]
                if 'Hinge' in connection.src.type:
                    print("Hinge orientation: ", connection.dst.orientation)
                else:
                    self.src.setHpr(0, 0, -heading)
